...
public:
   ...
   void drawCircle();
private:
   int x1, y1, x2, y2; // x1, y1 = center; x2, y2 = point on circle
};
...
void Circle::drawCircle()
{
   ...
   for(int i = 0; i < 100; i++) 
   {
      ...
      glVertex3f(x1 + radius * cos(angle), y1 + radius * sin(angle), 0.0);
   }
   glEnd();
}
...
{  
   ...
   for(int i = 0; i < 50; i++) 
   {
      ...
      glVertex3f(0.05*width + 0.02*width * cos(angle), 0.65*height + 0.03*height * sin(angle), 0.0);
   }
   glEnd();  
}
