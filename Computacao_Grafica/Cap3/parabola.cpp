...
void drawScene(void)
{
   ...
   for(i = -M; i <= M; ++i)
   { 
      glVertex3f(  50 /*usado para centralizar com a orta */ +
          50.0 /*usado para abrir o gráfico */ * (float)i/M,
         100.0 * (float)(i*i)/(M*M), 0.0 ); /*análise pega só um lado ai 2 for, nesse caso só um for*/
   }
   glEnd();
